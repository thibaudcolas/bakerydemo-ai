...
class Command(BaseCommand):
    ...
    def make_stream_field_content(self, title, subtitle, collection_name):
        ...
        streamfield_chat_completion = chat_completion.choices[
            0
        ...
        return [
            (
                "paragraph_block",
                RichText(json_streamfield_chat_completion["paragraph_1"]),
            ),
            ("image_block", {"image": image_1}),
            (
                "paragraph_block",
                RichText(json_streamfield_chat_completion["paragraph_2"]),
            ),
            ("image_block", {"image": image_2}),
            (
                "paragraph_block",
                RichText(json_streamfield_chat_completion["paragraph_3"]),
            ),
        ]
    ...
    def generate_images(self, theme, number_of_images, collection_name):
        for _ in range(number_of_images):
            ...
            if response.status_code == 200:
                ...
                wagtail_image = Image(
                    title=prompt, collection=collection, width=1024, height=1024
                )
    ...
    def get_random_image(self, model, collection_name):
        return (
            model.objects.filter(collection__name=collection_name).order_by("?").first()
        )
    ...
    def create_pages(self, page_count, theme, collection_name):
        ...
        blog_index = BlogIndexPage.objects.get(pk=84)
        for _ in range(page_count):
            ...
            BlogPersonRelationship.objects.create(
                page=new_blog_page, person=self.get_random_model(Person)
            )
    ...
    def handle(self, **options):
        ...
        generate_images = (
            input("Do you want to create new images? (y/n): ").lower().strip()
        )
        if generate_images == "y":
            number_of_images = int(input("How many images should be created? "))
            collection_name = input(
                "What should the collection to keep these images in be called? (Or what is the name of an existing one?) "
            )
        else:
            number_of_images = 0
            collection_name = input(
                "Which collection do you want to take images from? "
            )
